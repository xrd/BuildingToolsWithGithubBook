...
<2> `agithub` decodes the JSON from the API call into a Python dictionary.
    Here we're only really interested in the names of the organization, so we use a _list comprehension_, where we tell Python to only keep the value of the `"login"` field from each dictionary in the `data` list.
...
Let's write the first of the two panels we swap in and out.
...
However, our use case has a nontechnical user running this; let's see what can be done to make it easier for them to get started.
