...
Our initialization function starts by setting up some layout dimensions. Then, we create the UI controls and 
retrieve the credentials from the user using the `git crendentials fill` command and wrapper we described earlier. 
We then change the title and display the application to the user. 
...
Now let's discuss the implementation of our credentials code and then finally we will discuss the layout code we need. 
...
GridBagSizer nesting is a power feature of sizers, and allows almost any arbitrary layout to be possible -- although perhaps not easy or simple.
