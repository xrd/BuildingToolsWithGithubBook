...
$scope.annotateAfterForkCompletes = function() {
    $scope.forkedRepo = gh.getRepo( $scope.username, "spa.coffeete.ch" ); // <7>
    $scope.forkedRepo.read( "gh-pages", "cities.json", function(err, data) { 
        if( err ) {
            $timeout( $scope.annotateAfterForkCompletes, 10000 );
        }
        else {
            $scope.notifyWaiting( "annotating", "Annotating data on GitHub" ); // <8>
            // Write the new data into our repository
            $scope.appendQuirkToShop();
            $scope.forkedRepo.write('gh-pages', $scope.city.name + '.json', 
                    JSON.stringify( $scope.shops ), 'Added my quirky information', function(err) { // <9>
                if( !err ) {
                    ...
                    target = gh.getRepo( "xrd", "spa.coffeete.ch" ); // <11>
                    target.createPullRequest( pull, function( err, pullRequest ) { // <12>
                        if( !err ) {
                            $scope.notifyWaiting( "annotated", "Successfully sent annotation request" );
                            $timeout( function() { $scope.notifyWaiting( undefined ) }, 5000 ); // <13>
                            $scope.$apply();
                        }
                    } );
                }
...
$scope.notifyWaiting = function( state, msg ) { // <14>
    if( state ) {
        $scope.waiting = {};
