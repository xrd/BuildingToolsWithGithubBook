...
Now let's write the code which pulls out the information for the body and the title.

[source,ruby]
-----
[filename=".", language="ruby", sha="5ca9032:support/jekyll-parser/scraper.rb" lines="25..-4"]
snippet~~~~~
To be replaced
snippet~~~~~
-----

<1> First, we load up a VCR cassette, which just says "store any HTTP requests
...
the file using the YAML language).
...
<3> We then take the 5th (ruby uses zero-based offsets) row, and
process it using a method called `process_body` which, you guessed it,
processes the 5th row as the body content.
<4> The `process_body` method takes the retrieved DOM node and
converts it to pure text (as opposed to leaving us with the HTML tags)
and then strips whitespace from the front and end, and then prints the
first 50 characters along with the journal entry index.
...
-------
$ ruby run.rb
4: I'm now entering the my third day in Salvador.  Th
15: I had heard about the hill-tribes in 
northern Tha
22: On Sunday, Sept. 17 Jeanne picked up Vic at Jackie
23: Translation -  "Foreigners in India"Well since we 
24: April 1999Cuba, what an interesting place??!!
My a
25: Oct/Nov 2000"NEPAL"We spent our first 3 days in Ba
...
-------
...
We should grab the titles as well.  Add a method called
`process_title` and add that into the `get_ith_page` method underneath
the `process_body` method. 

[source,ruby]
-----
[filename=".", language="ruby", sha="e4f924c:support/jekyll-parser/scraper.rb" lines="9..38"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
