...
Let's start by writing a parser class. This will be helpful because we
can run instantiate the parser class inside a runner script, but we
can also instantiate the same class inside a test harness. As we will
find out, there are edge cases we will need to cover when scraping
data and converting it to Markdown, and having a testable class will
help with that.
