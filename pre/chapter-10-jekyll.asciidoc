...
[source,ruby]
-----
[filename=".", language="ruby", sha="2aa63e8:support/jekyll-parser/scraper.rb" lines="20..60"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
<2> Do the same with the title, but processing out the occasion when a
...
[source,ruby]
-----
[filename=".", language="ruby", sha="e19c1b2:support/jekyll-parser/scraper.rb" lines="20..55"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
Heredocs simply start with a tag and then end the template with a tag
("TEMPLATE" here), and everything in between is treated as a single
string. 
<3> Jekyll expects markdown files to have a specific format. We need
...
the `process_body` (inside the `get_ith_page`) call we added a call to
this function, giving it `i` and `row[3]` as the arguments.
