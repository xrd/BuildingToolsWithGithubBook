...
things to notice: the response is JSON by default, there are a lot of
URLs pointing to secondary information, and parameters are included in
the URLs. 
...
as AWK, to process JSON responses. However, there is a fantastic JSON
specific parsing tool which complements cURL that is worth knowing:
jq. For example, in the prior example, to retrieve the text from the rate
limit request:

[source,bash]
------
$ curl -u xrd https://api.github.com/rate_limit | jq '.rate.remaining'
4995
------

What just happened? The `jq` tool parsed the JSON, and using the
`.rate.remaining` filter, it retrieved content from the JSON
response. If you look at the response again, you'll notice it has
key/value pairs inside an associative array which has a single key:
`rate`. The filter we provided pulls out the information keyed by
`rate` and then find the key value pair keyed by `remaining` and then
prints the result. 

.Installing jq
[NOTE]
jq can be installed from source, using package managers like `brew` or
`apt-get`, and there are binaries on the downloads page. 
...
`-v` : print all request and response information.

`-i` : print just header information
...
Here we retrieve information about our rate limits, protected
information only available to a logged in user.
...
All of these problems are solved with oAuth. For these reasons, there
are very few reasons to use username and passwords. If you do need
simple and quick access to the GitHub API (and you don't use two
factor authentication) then HTTP basic authentication can help you in
a small subset of use cases.
...
require. Don't be confused by the following example which uses HTTP
basic authentication to create an oAuth token and asks for the
`public_repo` rights. If this token is properly issued, the oAuth
token will have permissions to read and write to public repositories
owned by that user.
...
scope, be sure to remove it from the list you provide to GitHub when
authorizing. 
...
* the service provider (GitHub) requests authentication: username and
  password usually.
* if two-factor authentication is enabled, ask for the OTP code.
...
* How to decode headers
* The two most important status codes: 200 and 304
* JSON Encoding
...
** GitHub API is great about following proper HTTP conventions
* Usage Limits
* ETag Usage
...
* Hypermedia
* User Agent required
** cURL automatically adds a "User Agent:" header
* ("User-Agent: curl/7.37.1"), but make sure your client library does!
* Versioning
** Use v3 for now
* Review enterprise appendix for more on how to use v3 mount point.
* Authenticated Access
** 404 does not mean you are doing it wrong, could be that the authentication is incorrect. 
* Username/Password
** SSH Keys
** oAuth tokens
** oAuth flow
...
Any of these service URLs can be used with the cURL tool to pull more
specific information from the GitHub API. For example, if we wanted to
know more about the "github" organization, we could use the following
cURL command: 
