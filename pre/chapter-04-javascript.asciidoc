...
[source,javascript]
-------
[filename=".", language="js", sha="4a46f27:support/js/karma.config.js"]
snippet~~~~~
To be replaced
snippet~~~~~
-------
...
-----
...
[filename=".", language="js", sha="779a83a:support/js/index.html" lines="8..37"]
snippet~~~~~
To be replaced
snippet~~~~~
...
...
[source,javascript]
-----
[filename=".", language="js", sha="2e51a82:support/js/coffeetech-annotate.spec.js", lines="1..38"]
snippet~~~~~
To be replaced
snippet~~~~~
...
...
-----
...
[filename=".", language="js", sha="987cdaf:support/js/coffeetech-annotate.spec.js", lines="40..-1"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
-----
...
[filename=".", language="js", sha="ba42181:support/js/coffeetech.js" lines="95..164"]
snippet~~~~~
To be replaced
snippet~~~~~
  ...
...
our tests, it takes a shop object to work on.
<2> We prompt the user three times: username and password on GitHub,
and the text they want to annotate.
...
<4> The GitHub.js library has you create a repository using the
...
existing array.
<7> Once we have verified the fork has completed, we need to get the
new forked repository. We use the username they provided when they
logged in to build the repository object. We then read the
...
<8> REWORK:When we re-enter the timeout callback, we setup a repository
object for our new forked repository and try to request some content
from it. If this succeeds, we know the fork completed.
<9> REWORK: We define a transformation function which we'll use in the next
...
<10> REWORK: With the fork ready, we write back into the repository with our
...
difficult. 
<11> To make a pull request, we create a repository object from the
Github.js library based on the original repository, and we issue pull
requests against that.
<12> If the pull request succeeds, we update our status messages and
are done.
<13> REWORK
<14> REWORK
...
code to do this at the end of the chapter. But, creating even a simple
authentication system has a baseline of complexity that seems like
overkill, and this code is complex and lengthy. If we could instead
delegate this authentication to a third party, we could reduce a
massive amount of code and complexity from our system. Let's see what
that looks like.
