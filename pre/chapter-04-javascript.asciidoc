...
Most GitHub API applications are some kind of code, be it Python or
Ruby or Go, running on a server. Usually you, as the author of said
code, will need to set up and maintain this server, and this can be an
onerous role. If you use, however, a special combination of branches within your 
repository and your code is hosted on GitHub, you can completely
...
pure JavaScript GitHub API library. It works well so let's use it to
build a single page applications hosted entirely on GitHub, no server
required.

Beyond saving us some money in hosting fees, there are other good
...
A single page application should server a single simple purpose. It
has to be important and direct. It should help us do something
important. Logically, this application should help us find coffee.
...
implementations and the test scripts.
