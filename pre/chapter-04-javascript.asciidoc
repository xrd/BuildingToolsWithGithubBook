...
another host, the user's web browser, and the only language ubiquitous
there is JavaScript. You might be surprised to know that there is a
pure JavaScript GitHub API library, and you might be surprised again to know
it works well for building real applications. Let's use the JS library to
build a single page application (SPA) hosted entirely on GitHub, and
save ourselves the headache of managing a server.
...
<body ng-app="coffeetech"> <3>
<div class="container" ng-controller="GithubCtrl">
{{ repo | json }} <2>
</div>
<script src="angular.js"></script> 
<script src="github.js"></script> 
<script src="coffeetech.js"></script> <1>
</body>
</html>
-------
...
<1> We added a reference to our `coffeetech.js` file beneath our other JS
references. 
<2> We removed our databinding to the `Welcome to CoffeeTech` string
and replaced it with a binding to the variable 
`repo` filtered by the JSON filter. 
<3> Finally, we changed the `ng-app` reference to use the module we
...
describe( "GithubCtrl", function() {
    ...
    describe( "#init", function() { // <7>
        it( "should initialize, grabbing current city", function() {
            scope.init(); // <8>
            expect( geo.getCurrentPosition ).toHaveBeenCalled(); // <9>
            expect( gh.getRepo ).toHaveBeenCalled(); // <10>
            expect( repo.read ).toHaveBeenCalled();
            expect( scope.cities.length ).toEqual( 2 ); // <11>
...
<4> Our scope is the object where all our functionality is held, and
by creating a new scope using the AngularJS utility function
`$rootScope.$new()` function we can then use the scope later to test
against the functionality we've implemented in our actual code.
...
object.
<6> Now we can write our tests. We create a describe block to group
them called `#init` as we are testing this function.
<7> 
...
[source,javascript]
-------
[filename=".", language="js", sha="021262697f6798e0912519a874161dad1cc3ed9a:support/js/coffeetech.js.7693187"]
snippet~~~~~
To be replaced
snippet~~~~~
-------
...
[source,html]
-----
[filename=".", language="js", sha="0e872b1b67dbd0c29ff42c211083f31add9c0123:support/js/index.html"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
[source,html index.html]
-----
[filename=".", language="js", sha="177c96ceb8dd4b21b8d66717abbac3c8a36cb4c7:support/js/index.html"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
-----
...
[filename=".", language="js", sha="3c3540fe6cf5dd97e536e9a7d7cfabbdca70d4d9:support/js/index.html" lines="8..29"]
snippet~~~~~
To be replaced
snippet~~~~~
...
...
[source,javascript]
-----
[filename=".", language="js", sha="6a1cbb8:support/js/coffeetech-annotate.spec.js"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
-----
  ...
[filename=".", language="js", sha="33451d6fda5fa5cd6e4de182bbe63c88b398b862:support/js/coffeetech.js" lines="92..164"]
snippet~~~~~
To be replaced
snippet~~~~~
  ...
