...
[source,javascript]
-----
[filename=".", language="js", sha="2e51a82:support/js/coffeetech-annotate.spec.js", lines="40..-1"]
snippet~~~~~
To be replaced
snippet~~~~~
-----

<3> According to the documentation for `fork` in the Github.js library, this method can
...
timeout in our app and query for the new repository. To get a handle
on the programmatic timeout, we need to use these statements to
retrieve the injector and then use the injector to get the `$timeout`
dependency. 
<2> This explains the
`$timeout.flush()`, a mock of the timeout browser call which
we can manually reconcile. 
