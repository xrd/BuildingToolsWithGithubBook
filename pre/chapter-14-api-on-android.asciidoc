...
[source,bash]
$ mkdir wia # Where I'm At == wia
$ cd wia
$ android create project --target 13 --name Wia --path . --activity MainActivity --package com.whereimat

These three commands create a new directory `wia`, enter the
...
directory called `./bin/Wia-debug-unaligned.apk`. To install it on
...
[source,groovy]
-----
[filename=".", language="groovy", sha="fde4fda:support/android/wia/build.gradle"]
snippet~~~~~
To be replaced
snippet~~~~~
-----

Gradle build files use some standard boilerplate, but there are two items of note here. 
...
ways to manage them. This application uses very simple forms, so we
will be creating our UI imperatively (using Java code).
...
[source,yaml]
-----
[filename=".", language="js", sha="4aa08622318:support/android/WhereImAt/features/my_first.feature"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
