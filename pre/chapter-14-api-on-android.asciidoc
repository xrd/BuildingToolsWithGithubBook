...
[source,yaml]
-----
[filename=".", language="js", sha="209cb6c:support/android/ghru/features/my_first.feature"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
-----
...
[filename=".", language="js", sha="67806ce:support/android/ghru/src/com/githubru/MainActivity.java" lines="15..-1"]
snippet~~~~~
To be replaced
snippet~~~~~
...
...
meaning starting it off the main UI thread.
<3> Here we define the derived AsyncTask class. The three types in the
generics signature provide a way to parameterize the calling context;
we need to provide a username and password when instantiating the
AsyncTask, and the first type signature allows us to pass Strings.
<4> Once inside the `doInBackground()` function, we instantiate a
...
to do that.
<5> We wrap the call to `getUser()` in a try block as the function
signature can throw an error (if the network were down, for example).
We don't really need the User object at all, but this call verifies
that our username and password are correct and we store the result of
the call in our return value.
<6> We renamed the `login()` function to more accurately reflect the
fact that when we call this, we are already logged into GitHub.
<7> If our login was a failure, either because of network failure, or
