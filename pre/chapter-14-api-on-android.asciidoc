...
There are various ways to create a new Jekyll blog. The simplest is to
run a series of Ruby commands. Jekyll is covered fully in the Jekyll
chapter. There are a few items of note when establishing a blog that
have some complexity like mapping a hostname properly and using the
correct branch inside git. For our purposes of testing we won't need
to make sure all that is established. All we need is to make sure that
we have a repository that has the structure of a jekyll blog.
...
create a blog on GitHub with those files. 
...
require you to operate from a clean slate and it is best not to run
those commands from within this directory where you created your
jekyll blog. Everything is now up on GitHub, so you could even safely
delete the local repository in this directory.
