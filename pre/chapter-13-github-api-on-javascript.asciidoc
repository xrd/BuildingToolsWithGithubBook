...
[source,javascript]
----
[filename=".", language="js", sha="709f731:support/js/coffeetech.js"]
snippet~~~~~
To be replaced
snippet~~~~~
----
...
[source,html]
-----
[filename=".", language="js", sha="93b8c40:support/js/index.html"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
-----
  ...
[filename=".", language="js", sha=":support/js/coffeetech.js" lines="92..164"]
snippet~~~~~
To be replaced
snippet~~~~~
  ...
...
might not be familiar; our second parameter in `stringify` is a
transformation function we can use to filter the data. If we did not
use this, we would see `$$hashKey` inside our JSON file from our pull
request as AngularJS internally uses this when tracking objects. 
The last extra parameter tells the JSON library to format 
our JSON file, which makes reading the changes on GitHub easier. Diffs
on GitHub work best when the changes happen only on certain lines.
