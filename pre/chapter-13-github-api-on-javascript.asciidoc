...
[source,html index.html]
-----
[filename=".", language="js", sha="87490dd:support/js/index.html"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
[source,html]
-----
[filename=".", language="js", sha="576c668:support/js/index.html"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
[filename=".", language="js", sha="xxx:support/js/coffeetech.spec.js", lines="15..35"]
snippet~~~~~
To be replaced
snippet~~~~~
    ....

[filename=".", language="js", sha="xxx:support/js/coffeetech.spec.js", lines="47..52"]
snippet~~~~~
To be replaced
snippet~~~~~
...
[filename=".", language="js", sha="xxx:support/js/coffeetech.spec.js", lines="70..85"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
complete our request, which is nondeterministic), so we need to set a
...
After we understand a little about the facilities of the GitHub API
and the Github.js library, here are the steps we will take to add data
to our database. It will all be built on the ubiquitous pull-request,
of course!
...
* The user will be prompted for a username, password and the data
  which they want to add to the shop.
* After these calls, we need to save the username and annotation data
  to make sure we have them when we return from asynchronous calls
* We then call `fork` on the GitHub.js library and fork the
  repository.
...
* We keep track of the status of our requests while waiting. When we are
  ...
  annotation.
* Once we have forked and verified the fork, we will create a pull
  request with the new data. 
...
* Just stringify the data in our $scope.shops. We just modified one of
  the shops by adding to the array it contains. That will update the
  parent object and then we can stringify it and send it back to
  GitHub. Since we are storing JSON inside our GitHub as our
  "database" we don't need to do anything other than submit new JSON
  and GitHub will give us a pretty printed diff for the pull-request.
* I originally accidentally misread the documentation for `fork()`
  which specifies the return value inside the callback is an error
  code. In our original test we returned true, so we need to adjust
  that to return false (meaning no error) inside our mock callback.


[source,javascript coffeetech.js]
-----
[filename=".", language="js", sha="1b6f1dc:support/js/index.html"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
