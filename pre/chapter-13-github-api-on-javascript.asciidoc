...
authentication has successfully completed, and this string here must
match what our application provides when initiating authentication
with GitHub.
...
[source,bash]
-----
GITHUB_CLIENT_ID=1234567890abcdefghijk GITHUB_CLIENT_SECRET=0987654321kmnopqrstuv node github-local-login.js
-----
...
* We could migrate this app into this NodeJS application, using EJS
...
* Finally, this app is not very testable. One of the reasons AngularJS
  ...
  and I found it very difficult to wrap this code inside of a piece of
  test code. 
...
application in JavaScript, but we'll find a different way to
...
Firebase offers AngularJS bindings (called "AngularFire") and offer an
...
First, create an account on Firebase. Once you have done this, create
...
`Firebase` constructor using a simple shim: 
...
[source,javascript]
-----
[filename=".", language="js", sha="cb28565:support/js/coffeetech-annotate.spec.js" lines="39..77"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
<3> We will inject these into our controller as specified by the
Firebase documentation. We call this method to generate the mocks and then change our `$controller`
instatiator to inject these mock objects instead of letting AngularJS
inject the real ones. 
...
we don't do that here, meaning the dialog is momentarily popped up
