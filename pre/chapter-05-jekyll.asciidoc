...
just scrape the site. Scraping the site itself works well, but to
reduce load on the archive, we'll cache our results using a gem called
`VCR` (typically used to cached results from hitting a web service
during test runs but perfectly capable here as well). 
...
We will write an HTML parser. We will build a parser class, and then
use this class inside a runner script. The runner script will utilize
the scraper class in the same way as our automated tests.

Let's start by writing the runner script.
...
Our scraper class looks like this.
...
We will Use the mechanize gem, a simple and powerful way to scrape
sites. Add these additional lines to the Gemfile using this command
...
particular URL). Note that ss a side effect of caching, things work
...
post. To make things simple we will make each post just contain the
...
This shows we can add comments inside of our YAML Front Matter when
needed.  
