...
respecting the very simple format required by Jekyll. I started as a
Perl programmer and always loved this quote from Larry Wall, the creator of Perl:
"We will encourage you to develop the three great virtues of a
programmer: laziness, impatience, and hubris." Let's accept our
inherent laziness and choose the second route. We'll write some code
to scrape a site and make a new Jekyll site.
...
Our runner has a simple interface, and this simple interface will
carry over to our tests.
...
[source,ruby]
-----
[filename="../final-scraper", language="ruby", sha="12260c3e1f32d5bcbe94:Gemfile"]
snippet~~~~~
To be replaced
snippet~~~~~
-----

Then, install our gems using the command:
...
We now have the rspec tool installed and associated libraries.
...
If we run this, we will see this output. Remember, we run using the
`bundle exec rspec scraper_spec.rb` command which makes everything run
inside the bundler context (and load our libraries from the Gemfile,
instead of the default system gems).
...
Use the mechanize gem, a powerful way to scrape sites. Add to the Gemfile.

[source,ruby]
-----
[filename="../final-scraper", language="ruby", sha="3e078f8:Gemfile"]
snippet~~~~~
To be replaced
snippet~~~~~
-----

Make sure to install using the `bundle` command.

[source,ruby]
-----
[filename="../final-scraper", language="ruby", sha="7ee2ec1:scraper.rb" callouts="1,5,9,14,15", callouts_prefix=" # #"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
well, both of which are valuable here.
<3> Initialize the variables to default values. We store the root of
the URL on Archive.org here.
<4> We modify our run method to loop 100 times.
<5> Generate a URL to the specific page, retrieve the page and then
print out the index in our loop plus the title of the page object.
...
from the site (as is the case with item #58, for example), so we
should be sure to filter out these from our final pages.

Now we can start thinking about how to scrape the interesting pieces
of content we need from the pages we have identified as valuable.
...
seen how Mechanize makes it simple to make a GET request. Now, let's
...
and IRB executes the command right there. Once the command executes, IRB prints the result;
the characters `=>` indicate the return value. Many return values are
complex objects: the return value when you use `scraper.agent.get` is
a Mechanize ruby object. This is a very large object, so printing it
out takes a lot of real estate, and we've abbreviated the majority of
it here, and will do that for many complex objects to save space when
discussing IRB.

Now that we have the page, how do we extract information from it? When
we used the Chrome inspector we saw a path to the post title
(represented there by the path `html body table tbody tr td font table
tbody tr`). This is a clue to finding the same content using
Mechanize. Mechanize has a nice piece of syntactic sugar that makes it
easy to search the DOM structure: the "/" operator. Let's try it. 

[source,bash]
-----
2.0.0-p481 :003 > page / "html body table tbody tr td font table tbody
tr"
 => [] 
-----

If things had worked, we would have seen a return value with the
content we wanted, but in this case we got back an empty
array. Unfortunately, the paths vary when the document is loaded into
a browser (the browser can customize the DOM or the server can send
slightly different data to the client). But, if we experiment with
similar paths, we will find what we need. IRB is a great place to
experiment with different combinations until we get the right one. It
helps to jump back and forth between Chrome and IRB, examining the
structure of the HTML inside Chrome and then testing a path in
IRB. Eventually, we come across this path:
...
text on the page with the resultant structures found using the
query. It is important to note that we had to strip whitespace from
the title to make it presentable. We can incorporate this into our
scraper code, but this is a good moment to think about how we can write
tests to verify this works properly. And, testing provides us with
another opportunity: using caching.
...
the server, and then prints again, stopping and starting. The content
from Archive.org does not change at all since the original site was
scraped years ago. It seems like a good opportunity to put a caching
layer between us and the code, reducing impact on Archive.org and
making our script run faster. In addition, if we structure our code to
make retrieval and processing happen independently, we can write tests
to verify the processing.

[source,ruby]
-----
[filename="../final-scraper", language="ruby", sha="c1eb33a:scraper.rb" callouts="1", callouts_prefix=" # #"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
[source,ruby]
-----
[filename="../final-scraper", language="ruby", sha="798360e:Gemfile"]
snippet~~~~~
To be replaced
snippet~~~~~
-----

And, run `bundle` to install the libraries.
...
As a side effect of caching, things work much faster. If we analyze
the time we save using the `time` command, we see these results.
...
[source,ruby]
-----
[filename="../final-scraper", language="ruby", sha="0d6d82c:scraper_spec.rb"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
-----
...
[filename="../final-scraper", language="ruby", sha="5841a2e:scraper.rb", lines="36..36"]
snippet~~~~~
To be replaced
snippet~~~~~
...
...
Now that we have our titles, we can generate an actual Jekyll
post. This will not have anything beyond the title, but setting up
the structure of files that make up a real Jekyll blog up will allow
us to use the Jekyll command line tool. This tool will spot check our
new content as we will see errors when incorrect Markdown content is
added to a post.

Let's create a Git repository for our files. When the Jekyll tool
...
generate Markdown posts, add a method called `write` which writes out
the processed information after we have retrieved and parsed the pages from Archive.org.
...
Now that we have a framework in place, we can fully scrape the site,
and write test coverage to make sure our processing works
properly. We will scrape the remaining textual pieces of content which
includes the body of the post, the author name, the location about
which the article was written, the creation date, and then we will
retrieve the images associated with the post. Then we will show how we
can creatively insert them into a Jekyll post. And, for each
component, we will assure our processing works by writing a test to
verify the processing is proper.

===== Body Content, Creation Date and Author



===== Location and Inline Maps in Jekyll
...
[source,ruby]
-----
[filename=".", language="ruby", sha="6be532b0bd4b2055847339c76b7496ef47d902da:support/jekyll-parser/scraper.rb" lines="75..-1"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
make HTTP requests but that we don't want captured. There are methods in VCR to
ignore requests, but using them would require heavily refactoring of our `get_ith_method`. 
