...
Use the mechanize gem, a powerful way to scrape sites. Add these
additional lines to the Gemfile using this command and the install the libraries.
...
value. Many return values are complex objects: the return value when
...
value with the content we wanted, but in this case we got back an
...
the client). But, if we experiment with similar paths, we will find
what we need. IRB is a great place to experiment with different
combinations until we get the right one. It helps to jump back and
forth between Chrome and IRB, examining the structure of the HTML
inside Chrome and then testing a path in IRB. Eventually, we come
across this query path: 
...
[source,ruby]
-----
[filename="../final-scraper", language="ruby", sha="c1eb33a:scraper.rb" callouts="2,3,12,17,23,27,35,40", callouts_prefix=" # #"]
snippet~~~~~
To be replaced
snippet~~~~~
-----

<1> We require the VCR gem. 
...
Now we can write out the files themselves.

[source,ruby]
-----
...
[filename="../final-scraper", language="ruby", sha="074311f:scraper.rb", lines="56..85", callouts="1,7,16,23,27" callouts_prefix=" # #"]
snippet~~~~~
To be replaced
snippet~~~~~
...
