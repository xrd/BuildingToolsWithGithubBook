...
which utilize and validate the same steps as are inside the . So, let's take this
secondary approach and create three files: the scraper class, the
...
irb(main):038:0> ( rows[2] / "td font" )[0].text()
=> "author: \n\nMD \n\n\nread more from this author | \nsee maps from this author\n"
irb(main):039:0> author = ( rows[2] / "td font" )[0].text()
=> "author: \n\nMD \n\n\nread more from this author | \nsee maps from this author\n"
irb(main):040:0> author =~ /author:\s+\n\n([^\s]+)\n\n/
=> 0
irb(main):041:0> $1
=> "MD"
-----
...
string. There is more than one way to get this information, of course,
...
[source,ruby]
-----
[filename="../final-scraper", language="ruby", sha="db09320:scraper.rb", lines="72..95", callouts="3,4,9,19,21" callouts_prefix=" # #"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
entire body content of a post to be included with the YFM. We can
filter this out using the `reject` method (which returns a copy of the
hash with any matching filter excluded).
<2> Then, we append the body content to generate the new rendered output.
...
the author information, and insert it into our processed hash. This
will then be inserted into our YFM automatically within the `render` method.

We can then run `bundle exec ./run.rb` to rewrite our post files.

==== Adding Images to Jekyll

Jekyll can host any binary files as well, and Markdown files can
include these assets. Let's add the images from the original site.
...
we should match the original colors as well. 
...
collaboration, and GitHub makes that easy.
...
Jekyll blog. By investigating the internals of a Jekyll post, we 
explained the intricacies of YAML Front Matter (YFM) and how
seamlessly you can mix and match HTML with Markdown syntax. Jekyll
blogs can utilize their own custom CSS and we've shown how easy it is
