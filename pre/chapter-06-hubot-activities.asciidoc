...
Here we use the heroku api URL as our webhook endpoint. Unfortunately, we
...
request back into GitHub by using the client id and secret generated
when we created the webhook. A cracker might be able to guess about
where our endpoint exists, but unless Heroku or GitHub is compromised,
...
we need to secure the webhook. GitHub provides a way to sign the data
inside the webhook payload. 

We need to make some modifications to our PR handler code. First, we will need to
retrieve the secure hash GitHub provides inside the request
headers. Then, we will need to calculate the hash ourselves using the 
secret we maintain internally. If these hashes match, then we know the
incoming request and JSON is truly from GitHub and not an
attacker. 
...
There is an attack vector when using HMAC cryptography which involves
asking a user to compare a string and then analyzing the time it took to
make that comparison. In certain cases, you can use the different
times to compare different strings as informtion useful in an
attack. Said another way, different
hashes, when using naive comparison operators like `==` provide
attackers with valuable information. To eliminate this risk, we use a module
called secure-compare that obscures this timing information when
...
a debugging message when the user response and send a message back to
...
user who we are interacting with (provided they accepted). If they
...
our targetted chat user accepted the request, and this is done using the GitHub
API bindings. We want to make this testable, so we should pass in the
...
should let the chat room know what happened which happens using the
reply method on our response object.

[source,coffeescript]
-------------
...
[language="json", sha="db082dc:support/slacker-hubot/spec/pr-delegator.spec.coffee", lines="63..-1"]
snippet~~~~~
To be replaced
snippet~~~~~
-------------

We add the following code to the end of our delegator extension that looks like this.

[source,coffeescript]
-------------
...
[language="json", sha="7de796b:support/slacker-hubot/lib/handler.coffee", lines="39..-0"]
snippet~~~~~
To be replaced
snippet~~~~~
-------------

We added an internal variable called `_GITHUB` where we will store a
reference to our instantiation of the GitHub API binding. Our
interface to the `setApiToken` call passes in the instatiation; this
method takes our oAuth token and the binding because we can pass in a
mocked binding inside our tests. It then authenticates against the
GitHub API. 
...
[source,coffeescript]
-------------
[language="json", sha="7de796b:support/slacker-hubot/scripts/pr-delegator.coffee"]
snippet~~~~~
To be replaced
snippet~~~~~
-------------
...
The `respond` method provided by the Hubot API is documented mostly by
...
Node provides a module called `util` which has useful utility
functions. One of them is `inspect` which will dig into an object and
...
This is our response object printed out. There is lots here: the full
set of information that our slack adapter organizes and uses
internally. Inside it all we can find information we need,
specifically the user name and email. With this information in hand,
we can properly comment on the pull request.

===== Comparing Slack usernames to GitHub usernames via the Organization API

Well, almost. If the username is the same on Slack and GitHub, then we
can simply use that. If they match exactly, let's assume they are the
same identity. We can use the organization API to look that up. If we
don't see 
