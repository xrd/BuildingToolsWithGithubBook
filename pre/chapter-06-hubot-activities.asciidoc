...
expectations of this method since we've got that covered in prior
...
next). Accepting triggers our Probot to contact GitHub and add a comment to the pull request noting
our chat user accepted the request, and this is done using the GitHub
...
in a mocked object. Declining the request means nothing happens. If we
...
should let the chat room know what happened.
...
Our delegator extension looks like this:
