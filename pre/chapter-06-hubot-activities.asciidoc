...
users and finds a random one, as long as it is not the probot. You
might notice there are some edge cases we should account for in our
tests (endless while loop anyone?) which are left as an exercise for the
reader. 
