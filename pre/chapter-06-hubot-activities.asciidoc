...
Let's build our own Hubot extension and customize Hubot to our needs.
...
reply with a string like `accept 112`. The Hubot can then interpret
this to mean they are accepting PR #112 and not the other pull request which the
hubot invited John to respond to ten seconds later. 
...
Our accept request handler has code which triggers our Hubot to
...
receiving (and faking data) coming a GitHub webhook. There is a
