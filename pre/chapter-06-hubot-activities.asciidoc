...
of our response to the calling client is the same. We don't want to
...
directly. Fortunately, Hubot comes with a built in HTTP client, which
works great for accessing information over HTTP. The Slack API returns
the list of members in the room currently; we can look over this list
...
PR? http://pr/1` (and the Slack client will format it as a clickable URL).

Unfortunately, our tests are now broken, and we now have this failure: `TypeError:
...
we need to secure the webhook. GitHub provides a way to assign a
secret to the webhook which, you guessed it, we will retrieve and use
inside our Probot. 
...
asking a user to compare a string and then using the time it took to
make that comparison as a way to attack secure hashes. Different
hashes, when using naive comparison operators like `==` provide
information back to attackers. To eliminate this risk, we use a module
...
-----
...
[language="coffeescript", sha="37cd462:support/slacker-hubot/spec/pr-delegator.spec.coffee", lines="47..-1"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
headers. This is where the GitHub webhook will encode the secure
hash sing the HTTP_X_HUB_SIGNATURE key. Inside our test we will need
to provide the same signature. We could duplicate our code from the
handler, or we could just run our tests once, watch for the
console.log output which says "Hash: H7612..." and copy this into our
tests. If we do this, the test will pass. We'll leave this as an
