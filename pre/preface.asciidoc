...
This is a storybook: a book containing stories about building software
applications. More specifically, software applications dependent on
a remote service API ("Application Programming Interface").

Using an API to back an application is a common practice today. APIs
provide a great pattern for making data accessible to the multi-screen
world. If your application is backed by a remote service API, the
...
Why the GitHub API? At this moment, there is no service that has
captured the hearts and minds of developers more than GitHub. Through
a unique combination of culture and engineering prowess, GitHub has
exploded in popularity and utility. The GitHub API is exceedingly
relevant if you are building software because you are probably using
GitHub to manage your software code. For those that aren't, you
might be using Git without GitHub, and the GitHub API is useful to
know there as well, as it layers the functionality of Git into a
networked programming interface.
...
And, there is a hidden, bonus benefit of learning the GitHub API which
is illustrated best by one of the tenets that made GitHub so
successful. This powerful idea can't be understated: build
your own tools whenever and where ever you can. GitHub was special
because it was started by engineers and the deep engineering roots
fostered a culture prioritizing automation over getting things done
manually. GitHub engineers were empowered to write code to automate
anything they did on the job. Having code perform tasks made it easy
to share those tasks: running a program is much easier and
error-resistant than documenting a process and training someone on
that process and documentation. Thus, on-boarding new engineers, for
example, was well documented and transparent. Sadly, at many places I
have worked, it often seems like many people are there solely to
create justifications for their employment (which means they pride
themselves on excessive grunt manual work) rather than focusing on
building tools that enable and empower their team. Learning the
GitHub API provides you with the incredible power to build your own
tools when you are managing software development processes. This is a
transformational technique for you and your teams. 

Each chapter in this book explores building tools using the
technologies found in the GitHub API or complementary related
technologies developed and used by GitHub.
