...
[source,coffeescript]
-----
[language="coffeescript", sha="58c3cbb:support/slacker-hubot/spec/pr-delegator.spec.coffee"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
access token before running the cURL command.

```
$ export REPOSITORY=isagh
$ export ACCESS_TOKEN=ad5a36c3b7322c4ae8bb9069d4f20fdf2e454266
$ export WEBHOOK_URL=HTTP://herokuapp.com.blah
$ export CONFIG="{\"url\": \"$WEBHOOK_URL\", \"content_type\": \"json\"}}"
$ curl -u "$ACCESS_TOKEN:x-oauth-token" -v -H "Content-Type: application/json" -X POST -d $CONFIG, https://api.github.com/repos/$USERNAME/$REPOSITORY/hooks

curl -usigmavirus24 -v -H "Content-Type: application/json" -X POST -d
'{"name": "cia", "active": true, "events": ["push"], "config": {"url":
"...", "content_type": "json"}}' https://api.github.com/repos/sigmavirus24/reponame/hooks

$ curl https://api.github.com/repos/$USERNAME/$REPOSITORY/hooks?access_token=$ACCESS_TOKEN
# View the webhook...

```

Use the heroku api URL as our webhook endpoint. Unfortunately, we
cannot do this in local mode (our Probot runs only on our laptop right
now, but review the Go chapter for an easy way to simulate this using
ngrok...).

We will need to make sure all requests are real requests from GitHub
and not a cracker attempting to maliciously.

* Authorization
* Using Hubot's HTTP server

Let's make sure our tests reflect this new functionality. 
