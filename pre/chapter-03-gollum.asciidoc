...
Gollum is the GitHub open sourced contribution to Wikis. Just as Git
has revolutionized collaborative editing of code, Gollum wikis layer
the benefits of Git onto a proven publishing metaphor. And, Gollum
wikis are tightly integrated with GitHub, such that you can quickly
build and associate a wiki with any repository, and create a
collaborative documentation system around any repository hosted on GitHub.
...
call something a Gollum repository. Finally, we will build a project
management tool which allows us to edit a Gollum wiki in an entirely
different way, but still publishes information into GitHub as a
regular Gollum wiki. 
