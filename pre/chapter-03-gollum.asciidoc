...
same image, and alsox adds a frame and alt text (helpful for
...
Gollum is built on top of Sinatra (a Ruby library for building HTTP
servers, discussed in the Gist chapter) and there are utilize authentication gems
like `omnigollum` that provide many authentciation options. You could
also write your own authentication handler and run gollum inside that.
...
This code extends the instance of Sinatra running as Gollum (cheekily
designated in the "Precious" namespace) and adds an authentication layer.
...
On many softwre teams there is a tension between the design
