...
`build.gradle` files). We will make two
changes to this file (documented below), and let's display the entire file
here to explain how it works.  

[source,groovy]
-----
[filename="../GhRu2", language="groovy", sha="f57a20c:app/build.gradle", callouts="1,4,13,24,27", callouts_prefix=" // "]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
<2> *Our first addition is here*: in order to write UI tests, we need to specify a test runner
called the `AndroidJUnitRunner`
<1> Android Studio automatically adds a configuration to our build
file which loads any JARS (Java libraries) from the `lib` directory,
and installs the standard JUnit test tools. 
<1> *Our second addition is here*: With the next six lines we install the
OkHttp library from Square (which gives us a simple interface to
making HTTP calls in our tests). We also install the EGit GitHub
libraries, and commons codec, which is an Apache Foundation library
that helps to encode content into Base64, one of the options for storing data inside
a GitHub repository using the API. Lastly, we install the espresso
libraries, the Google UI testing framework. The first line (of the
three libraries) installs the test runner which we configured above. 
...
from within the manifests directory. We need to make one change, to
...
Adnroid Studio created a default layout for us (called
...
First, make a directory to store the unit test. Unfortunately, at the
time of this writing, Android Studio makes it strangely difficult to
create the test directory for an Android gradle project, so jump
into a terminal and make a directory manually. 

[source,bash]
-----
$ mkdir -p app/src/androidTest/java/com/example/ghru
-----

Inside this directory, add a file called `GitHubHelperTest.java` that looks like the following.

[source,java]
-----
[filename="../GhRu2", language="java", sha="f4253e28ea4d3c084a:app/src/androidTest/java/com/example/ghru/GitHubHelperTest.java", callouts="3,7,16,20,24,29,33,39,45", callouts_prefix=" // "]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
When we generate our project, Android Studio creates Gradle wrappers
which allow us to run our commands from the command line. We can run
our test using the command below. 
...
com.example.GitHubHelperTest > testClient FAILED
...
[filename="../GhRu2", language="java", sha="2ef2fcf:app/src/androidTest/java/com/example/ghru/MainActivityTest.java", callouts="2,9,13,16,17,19,21,22,27", callouts_prefix=" // //", lines="2..-1"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
Now, run the tests with the command `./gradlew connectedAndroidTest`. Much like
before, as we have not yet implemented the code to back these tests,
we will get a failure report, similar to the one we saw with our unit tests.
...
great class for managing background tasks called `AsyncTask`. We
...
in.
<2> Then, install a click handler so that when we submit our post
information, we can start the process to create the post on GitHub.
...
experience with AsyncTask.

[source,java]
-----
[filename="../GhRu2", language="java", sha="0f68b1f4a0fd6c:app/src/main/java/com/ghru/MainActivity.java", lines="30..60", callouts="18", callouts_prefix=" // //"]
snippet~~~~~
To be replaced
snippet~~~~~
...
[filename="../GhRu2", language="java", sha="0f68b1f4a0fd6c:app/src/main/java/com/ghru/MainActivity.java", lines="70..-2", callouts="18", callouts_prefix=" // //"]
snippet~~~~~
To be replaced
snippet~~~~~
...
...
-----
package com.example;
[filename="../GhRu2", language="java", sha="5721c1c:app/src/main/java/com/example/ghru/GitHubHelper.java" lines="2..45"]
snippet~~~~~
To be replaced
snippet~~~~~
...
...
-----
...
[filename="../GhRu2", language="java", sha="a608f2d3df0ca9a21:app/src/main/java/com/example/ghru/GitHubHelper.java", lines="49..81", callouts="1,9,10,16,22,26,29,31" callouts_prefix=" // //"]
snippet~~~~~
To be replaced
snippet~~~~~
...
...
[WARNING] 
The base commit SHA (the parent of our new commit) is very important. If you do not
properly acquire the base parent SHA and were to push your commit
without indicating the parent SHA in the commit, you will effectively
perform a force push to the branch and the last commit will have no
parent. This means you will effectively lose history in your
repository, as the branch tip commit will have lost its connection to
previous commits. 
...
-----
...
[filename="../GhRu2", language="java", sha="2ef2fcf6c8758:app/src/main/java/com/example/ghru/MainActivity.java" lines="78..-1"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
