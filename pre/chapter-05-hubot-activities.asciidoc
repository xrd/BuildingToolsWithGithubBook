...
[source,coffeescript]
-----
[language="coffeescript", sha="e38eeca:support/slacker-hubot/spec/pr-delegator.spec.coffee", lines="1..20"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
-----
...
[language="coffeescript", sha="431888f:support/slacker-hubot/package.json" lines="19..24"]
snippet~~~~~
To be replaced
snippet~~~~~
...
...
{
  "url": "https://api.github.com/repos/xrd/isagh/hooks/3846063",
  "test_url":
  "https://api.github.com/repos/xrd/isagh/hooks/3846063/test",
  "ping_url":
  "https://api.github.com/repos/xrd/isagh/hooks/3846063/pings",
  "id": 3846063,
  "name": "web",
  "active": true,
  "events": [
...
[source,coffeescript]
-----
[language="coffeescript", sha="45bfe34:support/slacker-hubot/scripts/pr-delegator.coffee", lines="16..22"]
snippet~~~~~
To be replaced
snippet~~~~~
-----
...
Once this is complete, we can now create a pull request for our
repository from the command line and test our Probot. We'll first
create a new branch inside our repository, create a test file, and
then push that content into our repository. Once we have a new branch,
we can create a pull request from it using the API. We can reuse some
of our command line pieces from prior commands: reuse the CONFIG
variable to match the pull request format. The required fields are the
title and body for the pull request, the "head" key which matches the
name of the branch, and where to merge it to using the "base" key.
...
times to get it to work correctly. The examples here as are written
work right out of the box, but don't be fooled into thinking that it
all went exactly as we expected the first time around. As these are
commands you might want to perform multiple times as you are
experimenting, let's put the commands described in the prior paragraph
into a bash script that is generic and can be run multiple times. 

Create a new repository, and push it into GitHub.

THIS NEEDS TO BE A CHAPTER FOLLOWING THE INTRODUCTION TO HUB
...
git commit -m "Adding test file to test branchat $DATE" -a
git push origin $NEW_BRANCH
CONFIG=$(echo "{ "title": "PR on '$DATE'", "body" : "Pull this PR'$DATE'", "head": "'$NEW_BRANCH'", "base": "master" }')
curl -H "Authorization: token d71053e8d73ca9f6f59e6ffb4758e50093958894" -H "Content-Type: application/json" -X POST -d "$CONFIG" https://api.github.com/repos/$USERNAME/$REPOSITORY/pulls
------
...
{
  "url": "https://api.github.com/repos/xrd/isagh/pulls/1",
  "id": 27330198,
  "html_url": "https://github.com/xrd/isagh/pull/1",
  "diff_url": "https://github.com/xrd/isagh/pull/1.diff",
  "patch_url": "https://github.com/xrd/isagh/pull/1.patch",
  "issue_url": "https://api.github.com/repos/xrd/isagh/issues/1",
  "number": 1,
  "state": "open",
  "locked": false,
  "title": "A PR test",
...
This returns a huge JSON response, but you can see the first item is
the link to the pull request. Were we to visit this inside of GitHub,
we could merge the pull request from the web UI. If we then went to
the settings for our repository, and then clicked on the "Webhooks and
Services" link on the left navigation bar, at the very bottom of the
page we would see a list of recent deliveries to our webhook. 
