...
looking at the idioms found in many different languages, but each
chapter is designed so that you can follow it without intimacy to the
...
multiple basic tools to get things done. This chapter documents basic
operations of tools that will be extremely helpful when exploring the
GitHub API. 
...
* cURL: there are many tools that speak HTTP, and cURL is one of the
  best for its elegant and useful command line interface.

If you spend a little time installing these tools, you will have a
solid toolset for exploring the API.  
