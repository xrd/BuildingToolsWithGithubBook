...
It is tempting to write extensively about the API when used from a
tool like curl. But, the GitHub team has already done this: their API
overview includes examples of curl with almost every facet of the
overview. Bookmark this URL: https://developer.github.com/v3/
You'll use it often. Do note that this URL is tied, obviously, to the
current API "Version 3", so this URL will change when a new version is
released. 

Between this chapter and that URL you will have a good baseline for
how the API works. 
...
basic authentication supported by the `-u` flag in curl.

----
$ curl -i -u xrd https://api.github.com/rate_limit
...
oAuth has many variants, but GitHub uses oAuth2. 
