...
We are writing an application that adds Jekyll blog entries, and we
...
"secrets.xml" >> .gitgnore` is a quick way to add this to your _.gitignore_ file).
...
Our last step is to write the code that handles putting content into GitHub.
...
unit test and UI test that help to verify the functionality.
