...
image::images/btwg_07in01.png[Choose an Android SDK]
...
image::images/btwg_07in02.png[Reviewing the Android project structure for the first time]
...
on the app. The "res" directory contains the resources we
...
file that loads any JARS (Java libraries) from the `lib`
...
runner we configured above. We use `androidTestCompile`, which
...
[[creating-new-avd]]
.Creating a new AVD
image::images/btwg_07in03.png[Creating a new Android Virtual Device]
...
((("Android application example","default main for", id="ix_chapter-07-android-asciidoc6", range="startofrange")))When we use the above commands to create a new android application, it
...
layout. To do so, right-click (control-click on OS X) on the `layouts`
...
the file name, and accept the other defaults.
...
which repository to save into, to enter their blog post into
...
that the UI looks appropriate. Our login form looks like this:

[[simple-ui-blog-posts]]
.A simple UI for making blog post entries
image::images/btwg_07in04.png[A simple UI for making blog post entries]
...
((("Android application example","base SHA implementation")))((("SHA (secure hash algorithm)")))Now we implement((("directed acrylic graphs (DAG)")))((("retrieveBaseSha() function"))) `retrieveBaseSha()`. A Git repository is a directed
...
((("Android application example","master resource updating")))Our final step is to take the new commit SHA and update your branch
...
image::images/btwg_07in05.png[Creating a unit test configuration]
