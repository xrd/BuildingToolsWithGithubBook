...
GitHub is "social coding" and really promotes the people behind the code as much they promote the code. If you use the GitHub web based tools to add an issue, comment or documentation, you can enter in "@username" (called an "at-mention") to automatically associate another user with the issue or pull request. This person is then automatically subscribed to notifications for that issue or pull request. Internally GitHubber prefer notifications over email; subscribing to information rather than expecting an email saying "Hey, I just fixed this bug." Of course, if you are the recipient of an at-mention, you have full control in unsubscribing from those notifications, and you can globally assert that you don't want to be included in mentions if you find yourself overwhelmed by this method of communication. For most people inundated by massive amounts of email, "at-mentions" are a better way to involve people than sending yet another email.
