...
When the history of GitHub is documented, the Ruby language will
...
* `.ruby-version` or `.rvmrc`: These two files tell your application
  ...
  like .ruby-version and will properly use the correct version of Ruby if
  they see this file.
...
Though most chapters have focused on a specific programming language (aside
