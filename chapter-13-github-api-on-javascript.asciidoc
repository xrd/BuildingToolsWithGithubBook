...
Chrome 32.0.1700 (Mac OS X 10.9.1) GithubCtrl #init should initialize, grabbing current city FAILED
	...
	http://errors.angularjs.org/1.2.10/$injector/nomod?p0=coffeetech
	    at /Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:78:12
	    at /Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:1529:17
	    at ensure (/Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:1454:38)
	    at module (/Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:1527:14)
	    at /Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:3620:22
	    at Array.forEach (native)
	    at forEach (/Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:303:11)
	    at loadModules (/Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:3614:5)
	    at Object.createInjector [as injector] (/Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:3554:11)
	    at workFn (/Users/xrdawson/Projects/GithubBook/github.js.test/angular-mocks.js:2101:52)
	http://errors.angularjs.org/1.2.10/$injector/modulerr?p0=coffeetech&p1=Error%3A%20%5B%24injector%3Anomod%5D%20Module%20'coffeetech'%20is%20not%20available!%20You%20either%20misspelled%20the%20module%20name%20or%20forgot%20to%20load%20it.%20If%20registering%20a%20module%20ensure%20that%20you%20specify%20the%20dependencies%20as%20the%20second%20argument.%0Ahttp%3A%2F%2Ferrors.angularjs.org%2F1.2.10%2F%24injector%2Fnomod%3Fp0%3Dcoffeetech%0A%20%20%20%20at%20http%3A%2F%2Flocalhost%3A9876%2Fbase%2Fangular.js%3F1390611446000%3A78%3A12%0A%20%20%20%20at%20http%3A%2F%2Flocalhost%3A9876%2Fbase%2Fangular.js%3F1390611446000%3A1529%3A17%0A%20%20%20%20at%20ensure%20(http%3A%2F%2Flocalhost%3A9876%2Fbase%2Fangular.js%3F1390611446000%3A1454%3A38)%0A%20%20%20%20at%20module%20(http%3A%2F%2Flocalhost%3A9876%2Fbase%2Fangular.js%3F1390611446000%3A1527%3A14)%0A%20%20%20%20at%20http%3A%2F%2Flocalhost%3A9876%2Fbase%2Fangular.js%3F1390611446000%3A3620%3A22%0A%20%20%20%20at%20Array.forEach%20(native)%0A%20%20%20%20at%20forEach%20(http%3A%2F%2Flocalhost%3A9876%2Fbase%2Fangular.js%3F1390611446000%3A303%3A11)%0A%20%20%20%20at%20loadModules%20(http%3A%2F%2Flocalhost%3A9876%2Fbase%2Fangular.js%3F1390611446000%3A3614%3A5)%0A%20%20%20%20at%20Object.createInjector%20%5Bas%20injector%5D%20(http%3A%2F%2Flocalhost%3A9876%2Fbase%2Fangular.js%3F1390611446000%3A3554%3A11)%0A%20%20%20%20at%20workFn%20(http%3A%2F%2Flocalhost%3A9876%2Fbase%2Fangular-mocks.js%3F1388779666000%3A2101%3A52)
	    at /Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:78:12
	    at /Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:3648:15
	    at Array.forEach (native)
	    at forEach (/Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:303:11)
	    at loadModules (/Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:3614:5)
	    at Object.createInjector [as injector] (/Users/xrdawson/Projects/GithubBook/github.js.test/angular.js:3554:11)
	...
	TypeError: Cannot call method 'init' of undefined
	    at null.<anonymous> (/Users/xrdawson/Projects/GithubBook/github.js.test/coffeetech.spec.js:40:19)
------
...
<2> Set the username and repository. If you are putting this into
...
<3> We use the `read` method to pull file contents from the
repository. Notice we use the `gh-pages` branch since we are storing our
single page app and all the data there.
<4> Once our data is returned to us, it is simply a string. We need to
reconstitute this data back into a JavaScript object using the
`JSON.parse` method.
<5> Since we are calling outside of AngularJS and returning inside a
...
If you run this in your browser, you will not see the Portland coffee
shops data placed into the proper format. Something is broken, so
...
===== Find our current geolocation

OK, so we have successfully pulled data from the GitHub API and placed
it into our app. But, we want to make it so that we can detect where a
user is located and give them the right data. Let's add this to our
controller, and we'll start by adding a test to make sure we do it correctly.

===== Lookup our current city

Searchability of data stored within a GitHub repository is not a forte
of the GitHub API. Afterall, Git excels in storing data
efficiently, never promising to understand that data. Adding search to the client 
side is not much work, fortunately. We'll use the latitude and longitude to
calculate which city we are in, and if we find one, then request the
data from GitHub specifically for that JSON file. Once we have a
succinct and compact set of data local to our city, we can perform the
searching right inside our app with little overhead.

Practicing test driven development, let's add tests for our code. Add
this to the bottom of our coffeetech.spec.js file.
...
describe( "#getCity", function() {
        ...
        describe( "#portland", function() {
            beforeEach( inject( function( $controller, $rootScope ) {
                // Use Portland as the location
                geo = getGeo( CITIES[0] );
                window = { navigator: { geolocation: geo } };
                spyOn( geo, "getCurrentPosition" ).andCallThrough();
...
In this case we have two new describe blocks, one which tests that
calling `getCurrentLocation()` correctly calls into our geolocation
object on the window (the way to pull the geolocation coordinates from
a browser) and that once we have established our current latitude and
longitude that calling `detechCurrentCity()` correctly looks at our
list of cities and determines our real city.

Once we have our tests, we need to refactor our code a bit. Instead of
loading the portland.json file inside the `init()` function, let's
load a list of cities stored inside `cities.json`.

[source,javascript]
-----
[ { name: "portland", lat: 45, lng: 45 },
  { name: "seattle", lat: 47.662613, lng: -122.323837 } ]
-----

Then, we can load this file, determine our current latitude and
longitude, and match the city based on that list. If we find a match
within 25 miles, we can then load the correct JSON file for that city.
This is the way we will build searchability inside our data set, just
...
mod.controller( 'GithubCtrl', [ '$scope', 'Github', '$window', function( $scope, ghs, $window ) {
    $scope.init = function() {
    ...
    $scope.successfulGeolocation = function( position ) {
    ...
    $scope.getCurrentLocation = function() {
        if ($window.navigator.geolocation) {
            $window.navigator.geolocation.getCurrentPosition($scope.successfulGeolocation, $scope.geolocationError);
        } else {
            error('not supported');
        }
...
information themselves and help build our database. Add a button at
the top of the page which triggers display of an form for adding
information.  Add this snippet right beneath the div with the
`ng-controller` directive attribute.

[source,html]
-----
<button ng-click="add=!add">Add new coffeeshop...</button>
<div ng-show="add">
<form>
<fieldset>
<legend>Add new coffeeshop</legend>
<div class="control-group">
<label class="control-label input-label" for="name">Name:</label>
<div class="controls">
<input type="text" type="text" id="name" name="name" ng-model="addition.name"
placeholder="Enter name of coffeeshop..." />
</div>
</div>
</fieldset>
<button ng-click="addNewCoffeeshop(addition)
</form>
</div>

-----

Add a few methods to the end of our coffeetech.js file

[source,javascript coffeetech.js]
-----
$scope.addNewCoffeeshop( addition ) = function() {
  // Make a fork of this repository, and then add data to it.                         
  github.
}
-----
