...
mod.controller( 'GithubCtrl', [ '$scope', 'Github', 'Geo', function( $scope, ghs, Geo ) {
    ...
    $scope.detectCurrentCity = function() { // # <6>
...
showed that our distance was 571. In fact, I had forgotten this
algorithm actually calculated the distance in kilometers, not miles.
So, we need to multiply the result by 0.621371 to get the value in
miles, which ends up being close enough to what Google reports the
distance to be. 
...
<div class="row" ng-repeat="city in cities"> # <1>
