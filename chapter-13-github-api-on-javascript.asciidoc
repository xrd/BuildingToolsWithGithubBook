...
* Ruby: almost all GitHubbers use Ruby, and it will simplify our life
  here as we develop this app. Use rvm.io to install it.
...
===== Developing and using our site locally

====== Using Pow

If you are working on a Mac, you have the easiest setup. Mac users
have a superb simple webserver setup called `pow`. Pow facilitates
setting up rack compatible servers with a few commands and maps them
to a hostname running locally on your computer. If you feel
uncomfortable installing this software, use the commands for Windows
or Linux users.
...
====== Without Pow (Linux or Windows)

If you don't have a Mac or are unable or uncomfortable using Pow, then
just use a ruby one liner to serve all the files. This is a little
less efficient, but works.

[source,bash]
$ ruby -run -e httpd . -p 5000

Then, hit `http://localhost:5000/` with your browser to see the files.
...
Then, download the necessary files locally using these commands:
...
[source,bash]
$ git commit -m "Adding user information JS controller" -a
...
syntax, understand these features AngularJS, and then understand the 
problems solved by those features:
...
* Inspecting a JS object inside your webpage can be complicated; do
  ...
  all the marshalling we just realized is a sore point for most modern
  JavaScript developer lives? If we are using AngularJS it does not
  have to be.
  AngularJS provides a filter which you can apply with the pipe
  character that produces a pretty printed object in your webpage. You
  see that with the `repo | json` code.
* Many people see this kind of two way databinding and assume it
  ...
  by manually calling the $digest function on the $scope object.
...
If you load this up in your browser
