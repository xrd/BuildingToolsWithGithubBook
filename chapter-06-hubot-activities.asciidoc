...
Going to https://slack.com/[slack.com] starts the process of creating your own Slack
...
image::images/btwg_08in01.png[Creating the #generic channel]
...
image::images/btwg_08in02.png[Adding service integrations to Slack]

Slack supports many different service integrations, and one of them is
Hubot.

image::images/btwg_08in03.png[Choosing Hubot from the man Slack integration options]
...
image::images/btwg_08in04.png[Choose Hubot's name]
...
image::images/btwg_08in05.png[Hubot telling us the rules]
...
image::images/btwg_08in06.png[The built-in Hubot commands]
...
have Ruby set up as explained in the first chapter.
...
CoffeeScript. CoffeeScript is a intermediate language that compiles
...
express to match against and a function that receives an instance of
...
are using two-factor authentication or not) you will then receive an OAuth token.
...
image::images/btwg_08in07.png[A list of failed webhook deliveries]
...
Now that we have an incoming pull request (albeit one we are
...
The signature is a _hash message authentication code_ (HMAC). HMAC
...
installed by the `npm` tool in the `node_modules` directory, and we
...
`hear` we could have used. `respond` is used when the message
