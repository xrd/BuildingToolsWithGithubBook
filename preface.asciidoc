...
* <<introduction>>: the chapter you are reading now covers a
...
* <<chapter2>>: covers the Gist API, as well as command-line
  ...
  displays gists.
* <<chapter3>>: explains usage of the Gollum command-line tool
  ...
  to GitHub.
* <<python_search_api>>: we explore the Search API and build a GUI
  tool to search repositories on GitHub using Python.
* <<commit_status>>: covers a relatively new part of
