...
Let's first work on the `login()` method. Poking into the https://github.com/eclipse/egit-github/tree/master/org.eclipse.egit.github.core:[Egit libary
reference], we can write GitHub login code that is as simple as the following.
...
tree. To determine this address, our applications needs to a reference to the
repository, and we use the repository service we instantiated
earlier. Once we have the repository, we need to look inside the
