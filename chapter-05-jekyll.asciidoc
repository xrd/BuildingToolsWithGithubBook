...
comes with a _.gitignore_ file as well that contains one entry:
pass:[<code>_site</code>]. When you use the Jekyll library to build your site locally,
all files are by default built into the +_site+ directory.
This _.gitignore_ file prevents those files from being included inside
