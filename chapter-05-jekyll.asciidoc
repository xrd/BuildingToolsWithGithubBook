...
`_site` (((range="endofrange", startref="ix_chapter-05-jekyll-asciidoc4")))directory.
====
...
[[bare-jekyll-site]]
.A bare Jekyll site
image::images/btwg_0601.png[A bare Jekyll site]
...
[[a-sample-post]]
.A sample post
image::images/btwg_0602.png[A sample post co-authored by Tom Preston-Warner]

Our Jekyll initialization command created this new post for us. This page
is backed by the Markdown file inside the `_posts` directory we
saw earlier.
...
((("CNAME file")))((("Jekyll blogs","and CNAME file")))The CNAME file is a simple text file with the domain name inside of
it.
...
`github` gem, which is no longer actively maintained but is still a useful command
line tool.
...
If your blog is correctly set up, you will see something like <<settings-jekyll-blog>>
in the middle of your settings page.

[[settings-jekyll-blog]]
.Settings for a Jekyll blog
image::images/btwg_0603.png[Settings for a Jekyll blog]
...
((("subdomain, DNS setup with")))It is easiest if you are setting up DNS with a *subdomain* as opposed
to an *apex domain*. To be more concrete, an ((("apex domains")))apex domain is a site
...
GitHub needed to change their IP addresses (say during a denial-of-service attack), you would need to respond to this, and deal with the
...
import.jekyllrb.com to import. What if we have a nonstandard site
we need to import?
...
image::images/btwg_0604.png[The calendar view for Archive.org ]
...
image::images/btwg_0605.png[Archive of Bytravelers.com on Archive.org]
...
((("Jekyll blogs","master index file creation with Liquid Markup")))((("Liquid Markup","master index file creation with")))We now have the posts generated properly, but we don't have an entry
page into the blog. ((("index.md file")))We can create a `index.md` file, which just
...
image::images/btwg_06in01.png[Austere but a step in the right direction]

This index page lists every post: let's make it display only the last
10 posts. Copy the `index.md` file to a file named `archive.md`.
...
((("CSS","for Jekyll blogs", id="ix_chapter-05-jekyll-asciidoc19", range="startofrange")))((("Jekyll blogs","custom CSS for", id="ix_chapter-05-jekyll-asciidoc20", range="startofrange")))We used ((("Bootstrap")))Bootstrap in the JavaScript chapter and will use it again
...
_default.html_ layout to make the site really standout. Many Jekyll
...
image::images/btwg_0606.png[Restoring the original images and colors]
