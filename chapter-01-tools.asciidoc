...
every major programming language, and often multiple client options
for each of the many languages. This book will take you on an
...
you install and play with those languages, but the story behind the
projects we will build will be interesting even if you don't actually
type a line of code in a language within this book you are
disinterested in learning on its own.

There are two languages and one tool, however, which stand above the
rest which you should actually install and use in order to get the
most out of this book. These tools are:
...
discuss the history of these ools within the GitHub story as well as
...
is the rawest viewpoint on the GitHub API you can find beyond a
network traffic analysis tool. cURL, like the HTTP protocol which
it speaks intimately, is stateless, meaning it is challenging
...
that solves this problem, but note that cURL works best with one-off
requests.
