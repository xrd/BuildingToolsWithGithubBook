...
it, look for the Clone URL to the right of the Gist itself. You should
see a link like:

https://gist.github.com/8138797.git

This is a HTTPS clone URL. Though you use this URL, updating Gists
from this URL is more troublesome. For example, if you are using two
factor authentication with your account, HTTP Basic authentication will fail. 

[source,bash]
-----
$ # Make some changes 
$ git commit -m "Made some changes" -a
$ git push
Username for 'https://gist.github.com': xrd
Password for 'https://xrd@gist.github.com': 
remote: Invalid username or password.
fatal: Authentication failed for 'https://gist.github.com/8138797.git/'
-----

If you don't use two factor authentication, it is still bothersome to
re-enter your username and password each time you push to the
repository. Though there are ways you can mitigate re-entering in 
your username and password each time you push (using `git config
credential.helper store`), there is a simpler way to allow updates:
use the SSH URL.
...
Notice that you use a different protocol (git) with an `@` symbol
between the hostname, and then a colon as the delimiter between the
hostname and the path to the repository.
