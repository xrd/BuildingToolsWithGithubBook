...
[source,cs]
----
Get["/authorize", true] = async (parms, ct) => // <0>
    {
...
<0> This is how you map paths to handler functions in Nancy.
...
<4> Here we redirect the user back to what they were originally trying to do, with as little disruption as possible.
