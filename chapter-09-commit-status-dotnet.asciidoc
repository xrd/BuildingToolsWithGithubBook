...
[NOTE]
====
The `await` keyword bears special mention.
It was introduced in .NET 4.0, in 2012, and it lets you write asynchronous code as though it were synchronous.
The above code will actually return a promise from the line that includes the `await` keyword, and the event loop will pick up the execution on the following line once the promise is fulfilled.
