...
Choose the search result named "Nancy.Templates", which belongs to the NancyFx organization, and click "Get Now."
This should download a `.vsix` file, which you can double-click to install the templates into Visual Studio.
...
This last endpoint is something we can test.
Here's a start on that, that covers the handling of the CSRF token.
In order to handle sessions, we need to add this snippet to our test project's namespace:
...
This is an _extension method_ which allows us to provide a `Session` object for a request, something the CSRF handling uses.
...
The second part has matching tokens, so we assert that the response is a redirect to the URL we put into the Session.
