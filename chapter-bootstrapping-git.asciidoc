...
Agit is an open source Android client for GitHub. Agit is developed by Roberto Tyley who built this client and then was hired by GitHub (and has since left for the Guardian). Agit is a read-only client, loading the entirety of any repository available to you on GitHub onto your phone. Agit displays diffs within the repository, and allows you to view branches within any repository you have cloned. 
...
Teddy Hyde is an Android client for GitHub specifically well suited for editing Jekyll blogs. Teddy Hyde was also written by one of the authors of this book, Chris Dawson. Teddy Hyde is a read-write client, and 
