...
This book is focused on Github, but before we can explore Github you have
...
a commit.  Contrast this approach to that of Subversion where branchs
...
upward trend the begins in 2008 and continues to today, and also notice a slight downward trend 
...
traditionally a web client-side language, on the server side, emphasizing providing developers
a set of tools for developing speedy and familiar web applications. Peering into the graphs 
underneath the project level commit graph, you see the project's creator, Ryan Dahl, initially
made a large majority of the changes in the project. As he has stepped back, others like "isaacs"
and "bnoordhuis" have taken over and are now making the majority of changes. Git and GitHub
make these transitions of ownership seamless and without argument.
