...
Git is not the only popular DVCS, but it is the most popular. Here is a quick comparison of the other major DVCSes, both technically and culturally.
...
Brew is a useful tool for installing lots of developer centric software. And, many of these components require installation of Apple's XCode development environment. Unless you feel comfortable with 
...
Linux systems make installation of Git a breeze. Most Linux systems use a package management system like dpkg (the Debian packaging, favored by Ubuntu and Debian) or RPM (the RedHat Package Manager, used by CentOS and other RedHat variants).
...
If you are committed to building a binary optimized to your exact machine, you can build Git from source. To do this, run these commands from a terminal:
